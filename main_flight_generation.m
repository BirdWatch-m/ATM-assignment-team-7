...
distanceMatrix = buildDistanceMatrixLocal(airportCatalog);

% generate flights
flights = generateRandomFlightsLocal(airportCatalog, distanceMatrix, aircraftTable, numFlights, minDistanceNm, maxDistanceNm);
...
v_cru2_kts = rawTable.("V_CRU2_kts");
if iscell(v_cru2_kts) || isstring(v_cru2_kts)
    v_cru2_kts = str2double(string(v_cru2_kts));
end

m_cru = rawTable.("M_CRU");
if iscell(m_cru) || isstring(m_cru)
    m_cru = str2double(string(m_cru));
end

h_mo_ft = rawTable.("h_mo_ft");
if iscell(h_mo_ft) || isstring(h_mo_ft)
    h_mo_ft = str2double(string(h_mo_ft));
end

wake_turb_cat = rawTable.("wake_turb_cat");

aircraftTable = table( ...
    aircraft_type, aircraft_category, bada_id, ...
    min_runway_length_ft, min_runway_width_ft, ...
    v_cru2_kts, m_cru, h_mo_ft, wake_turb_cat, ...
    'VariableNames', { ...
    'aircraft_type', 'aircraft_category', 'bada_id', ...
    'min_runway_length_ft', 'min_runway_width_ft', ...
    'v_cru2_kts', 'm_cru', 'h_mo_ft', 'wake_turb_cat'});
...
function distanceNm = greatCircleNmLocal(lat1Deg, lon1Deg, lat2Deg, lon2Deg)
...
%% Distance matrix — precompute all pairwise great-circle distances once

function distanceMatrix = buildDistanceMatrixLocal(airportCatalog)
n = numel(airportCatalog);
distanceMatrix = zeros(n, n);

for i = 1:n
    for j = i+1:n
        d = greatCircleNmLocal( ...
            airportCatalog(i).latitude_deg, airportCatalog(i).longitude_deg, ...
            airportCatalog(j).latitude_deg, airportCatalog(j).longitude_deg);
...
%% Flight time calculation — ISA atmosphere + crossover altitude (ported from BADA spec)

function flightTimeHours = computeFlightTimeLocal(distanceNm, vCru2Kts, mCru, flightLevel)
altitudeFt = flightLevel * 100;

if altitudeFt <= 36089
    tempK = 288.15 - 0.0019812 * altitudeFt;
    delta = (1 - 0.00000687559 * altitudeFt) ^ 5.25588;
else
    tempK = 216.65;
    delta = 0.22336 * exp(-0.0000480634 * (altitudeFt - 36089));
end

speedOfSoundKts = 38.96785 * sqrt(tempK);

% TAS from cruise Mach
tasMach = mCru * speedOfSoundKts;

% TAS from cruise CAS — convert CAS to Mach at altitude, then to TAS
a0 = 661.4786;
term1 = 1 + 0.2 * (vCru2Kts / a0) ^ 2;
term2 = (term1 ^ 3.5 - 1) / delta;
machFromCas = sqrt(5 * ((term2 + 1) ^ (1 / 3.5) - 1));
tasCas = machFromCas * speedOfSoundKts;

% Below crossover altitude TAS is CAS-limited; above it is Mach-limited
tasKts = min(tasCas, tasMach);

flightTimeHours = distanceNm / tasKts;
end


% Generate random flights function
function flights = generateRandomFlightsLocal(airportCatalog, distanceMatrix, aircraftTable, numFlights, minDistanceNm, maxDistanceNm)
...
distanceNm = nan(numFlights, 1);
flightLevel = nan(numFlights, 1);
flightTimeHours = nan(numFlights, 1);
...
for flightIndex = 1:numFlights
    ...
    for attempt = 1:maxAttemptsPerFlight
        ...
        [arrivalCandidates, candidateDistances] = findArrivalCandidatesLocal( ...
            distanceMatrix, departureAirportIndex, aircraftRow, airportCatalog, minDistanceNm, maxDistanceNm);
        ...
        chosenDistanceNm = candidateDistances(arrivalChoice);
        chosenFlightLevel = randi([100, floor(double(aircraftRow.h_mo_ft) / 100)]);
        ...
        badaId(flightIndex) = aircraftRow.bada_id;
        ...
        distanceNm(flightIndex) = chosenDistanceNm;
        flightLevel(flightIndex) = chosenFlightLevel;
        flightTimeHours(flightIndex) = computeFlightTimeLocal( ...
            chosenDistanceNm, double(aircraftRow.v_cru2_kts), double(aircraftRow.m_cru), chosenFlightLevel);
...
flights = table( ...
    flightId, aircraftType, aircraftCategory, badaId, ...
    adep, adepName, depRunway, ades, adesName, arrRunway, ...
    distanceNm, flightLevel, flightTimeHours, ...
    depLatitudeDeg, depLongitudeDeg, arrLatitudeDeg, arrLongitudeDeg, ...
    'VariableNames', { ...
    'flight_id', 'aircraft_type', 'aircraft_category', 'bada_id', ...
    'adep', 'adep_name', 'dep_runway', 'ades', 'ades_name', 'arr_runway', ...
    'distance_nm', 'flight_level', 'flight_time_hours', ...
...
function [arrivalCandidates, candidateDistances] = findArrivalCandidatesLocal(distanceMatrix, departureAirportIndex, aircraftRow, airportCatalog, minDistanceNm, maxDistanceNm)
arrivalCandidates = [];
candidateDistances = [];
...
for i = 1:numel(airportCatalog)
    ...
    distanceNm = distanceMatrix(departureAirportIndex, i);

    if distanceNm < minDistanceNm || distanceNm > maxDistanceNm
        continue
    end
    ...
    candidateDistances(end + 1) = distanceNm; %#ok<AGROW>
end
end
